...
export const generatePdf = asyncHandler(async (req, res) => {
  ...
  await page.goto(`${process.env.CLIENT_URL}/print`, {
    timeout: 0,
  ...
  await page.addStyleTag({
    content: `@page { size:${divDimensions.width + 2}px ${
      divDimensions.height + 2
    }px; }`,
  });
  ...
  for (let i = 0; i < excelData.length; ++i) {
    if (
      excelData[i].EMAIL == "" ||
      !excelData[i].EMAIL ||
      !isValidEmail(excelData[i].EMAIL)
    ) {
      continue;
    }
    ...
    await Certificate.findByIdAndUpdate(cert.id, {
      url: webViewLink.data.webViewLink,
    });
  ...
  res.status(200).json({
    message: `Job successfully completed, ${creditsUsed} credits used`,
  });
});
...
export const getHistory = asyncHandler(async (req, res) => {
  const history = await Certificate.find({ issuedBy: req.user.username }).sort({
    _id: -1,
  });
